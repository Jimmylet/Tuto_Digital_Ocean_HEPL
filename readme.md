...
Indiquez dans les 3 premier champs « Nom du serveur DNS » les serveurs suivant :  
...
À cette étape-ci, vous devriez être en mesure de réaliser un ping vers votre serveur. Ouvrez-votre terminal et tappez :
...
2. La première fois que vous tenterez la connexion, vous aller reçevoir un avertissement. Tappez `yes`
3.  On vous demande un mot de passe. Prenez celui de l'email dont je vous ai parlé précédemment. Coller le dans l'invite de commande.
4.  Coller le à nouveau.
...
*Avant de commencer à vous expliquer les étapes, sachez que je ne les aient pas tester travaillant principalement sous MAC.*
...
Pour installer le plugin qui permettra à Ajenti d'installer et de lire des sites webs sur le serveur, connectez-vous en SSH à votre serveur et tappez-y la commande suivante :
...
Ensuite, tappez dans le terminal : `brew install ssh-copy-id`
...
Et voilà, vous pouvez a présent envoyer des mails sur Gmail, Yahoo… et vos destinataires les recevront. Pour HOTMAIL (toujours Hotmail), ça bug toujours, allez comprendre pourquoi… Enfin, si un utilisateur d'Hotmail vous envoie un message, il le recevra correctement !
